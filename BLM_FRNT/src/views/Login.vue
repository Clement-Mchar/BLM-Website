<template>
  <Layout>
    <div class="flex flex-col min-h-96 items-center">
      ...
      <div class="flex flex-col min-w-[50rem] min-h-[25rem] justify-between border-white border-solid border-2 p-3">
        <CsrfHandler />
        <form class="min-h-40" @submit.prevent="handleSubmit">
          <div class="mb-4">
            <label for="username" class="block text-3xl pb-2 font-medium text-white">Pseudo</label>
            <input
              id="username"
              type="username"
          ...
          <div class="mb-4">
            <label for="password" class=" mt-8 pb-2 block text-3xl font-medium text-white">Mot de passe</label>
            <input
              id="password"
              type="password"
              v-model="form.password"
              required
              class="mt-1 block w-full h-16 p-2 text-2xl bg-slate-300 focus:border text-black rounded-lg shadow-xs focus:outline-hidden focus:ring-indigo-500 focus:border-indigo-500"
